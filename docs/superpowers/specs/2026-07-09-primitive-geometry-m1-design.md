...
```text
create_primitive(kind) -> EntityId
```
...
- 创建 primitive 保持当前 `create_cube()` 的 infallible 调用形态，不为了理论上的 id 耗尽把 UI/model 创建路径改成 `Result`。
- `IdGenerationExhausted` 不属于本 milestone 的 primitive 创建验收。若实现时顺手修正现有 `next_cube_id()` 饱和后的死循环风险，应作为局部 helper 安全修补，不改变 M1 的用户可见创建 API。
...
- `crates/editor/src/app/tests.rs` 当前显式断言 menu bar 不包含 `Create` menu；M1 必须把该测试改为期待 `File`、`Edit`、`Create`、`View` 都存在。
- toolbar source/test 中当前只覆盖 `Cube` 创建入口；M1 必须扩展到 `Cube`、`Sphere`、`Cone`，并确认三个入口都走 `EditorUiAction`。
...
- `viewport_draw_call_with_view_and_meshes` 仍是 primitive 和 imported mesh 的唯一 viewport draw-call 入口。
- `Cube`、`Sphere`、`Cone` 可以在 render 内部通过一个小的 primitive dispatch 选择几何生成函数，但生成后的 vertex/index/span 必须进入同一个 `ViewportDrawCall.vertices`、`ViewportDrawCall.indices` 和 `ViewportDrawCall.mesh_spans`。
- 不新增 `cube_spans`、`sphere_spans`、`cone_spans`、primitive-specific hit test、primitive-specific Fit View 或 primitive-specific gizmo path。
- unknown `primitive:*` 只在 render primitive dispatch 处被跳过，不能让 viewport/editor 其他层分叉处理。
...
- `render`：几何差异必须被直接测试，不能只看 mesh count、index count 或 span。测试要证明 sphere/cone 的 vertex/index topology 与 cube 不同，例如固定 vertex/index 数、position set、apex/base-ring 特征或 silhouette 特征至少覆盖一种明确断言。
- `render`：sphere 和 cone 之间也必须有明确差异断言，避免二者共用同一错误 preview mesh。
...
- editor smoke：创建并保存/重开三种 primitive，summary 至少证明 mesh count、viewport index count、content reopen 和 viewport prepare/paint。几何正确性不靠 smoke summary 证明，必须由 targeted render tests 证明。
