...
- Player 的 native lifecycle、window 和 winit event adapter 位于 `sge-player`。
- WGPU adapter、surface 和 GPU resources 位于 `sge-render`，由具体 host 提供 presentation target。
...
- Player 使用同一 renderer，只由 winit host 提供 surface。
