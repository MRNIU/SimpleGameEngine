...
`GameDescriptor` 合同类型由下层 `sge-app` 定义。game package 只负责构造一个静态 descriptor，至少包含稳定 `game_id` 和创建已配置 EngineApp 的函数：
...
game-specific binary 在 composition root 把 descriptor 显式传给 `sge_editor::run`、`sge_player::run` 或 `sge_build::build`。可复用 host 只依赖 `sge-app` 中的合同，永远不依赖具体 game crate。
...
flowchart TD
  ...
  Game --> App["sge-app"]
  Game --> Input["sge-input"]
  Game --> Math["sge-math"]
  Game --> Reflect["sge-reflect"]
  Game --> Scene["sge-scene"]
  Game --> Render["sge-render"]
```

通用 `sge` 是 `sge-build` package 内的另一个 binary target。它通过子进程构建并启动 `demo-game-build`，两者之间不是 Rust library dependency。

不创建 `simple-game-engine` facade。当前 game library 对 app/input/math/reflect/render/scene 的直接依赖分别承载真实组件、系统和注册职责；facade 只会镜像 re-export、隐藏实际依赖且没有独立 owner。若将来多个 game 出现稳定且更窄的共同 API，再以真实调用方另立规格。
...
| `sge-editor` | eframe host、EditSession、PlaySession、panels、Inspector drawers、文件和 build UI | ECS storage 实现、第二个 winit loop、游戏代码所有权 |
...
- Editor 先消费菜单、快捷键、viewport camera 和 UI events。
...
- editor-only viewport 和 panel state。
...
| Public composition | game/editor/player/build packages、headless integration tests |
...
M4 Render And Hosts、M5 Editor Play、M6 Build And Stage、M7 Integration Demo已完成，并删除被替代
prototype。最终demo只组合M1–M6公开能力，没有引入demo-only engine shortcut。
