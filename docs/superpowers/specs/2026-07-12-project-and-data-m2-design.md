...
`instantiate(prepared, initializer)` 只消费已经验证和 decode 的 `PreparedScene`，返回
`SceneEntityId -> Entity` 的 `SceneInstance`。任何 unexpected ECS/type error 使 candidate
app 无效并由调用方丢弃；不能继续使用半实例化 candidate。
