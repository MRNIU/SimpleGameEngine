...
M1 必须显式拒绝把当前开发仓库根作为用户 project。

仓库根判定用于防误写，不是通用安全 sandbox。实现可以用当前 repo 的稳定特征判定：

- 目录下存在 `Cargo.toml`。
- 目录下存在 `crates/`。
- 目录下存在 `AGENTS.md`。
- 目录下存在 `assets/primitives/`。

New Project 或 Open Project 的候选目录命中该判定时必须失败，并显示 `Repository root cannot be used as a user project`。测试必须覆盖创建和打开两个入口。
...
2. 目标目录命中 Repository Root Guard 时拒绝。
3. 目标目录为空时，editor 创建 project。
4. 目标目录已包含 `project.sge.ron` 时，New Project 拒绝，提示使用 Open Project。
5. 目标目录非空且没有 `project.sge.ron` 时，New Project 拒绝，避免覆盖用户目录。
6. editor 写入 `project.sge.ron`。
7. editor 创建 `scenes/main.scene.ron`、`assets/asset_manifest.ron` 和 `assets/imported/`。
8. current project root 切到该目录。
9. current scene 切到 default scene。

### Open Project

1. 用户选择 project directory 或 `project.sge.ron`。
2. 候选 project root 命中 Repository Root Guard 时拒绝。
3. editor 读取 project file。
4. version/name/default_scene 验证通过后，current project root 切换。
5. default scene 存在则打开。
6. default scene 缺失则创建默认 scene。
7. manifest 缺失则创建空 manifest。
8. manifest 解析失败则 Open Project 失败，不切换 current project。
9. asset cache 从 current project root reload。
...
- New/Open Project：absolute dialog path canonicalize 后作为候选 project root。
...
| 候选 project root 是仓库根 | New/Open Project 失败 |
...
- New Project 和 Open Project 都拒绝仓库根。
...
- repo sample project 从 `examples/projects/editor_smoke/` 作为 README/runtime/test 默认示例。

### Sample Project Migration

M1 必须消除示例 truth surface 分裂：

- 新增 `examples/projects/editor_smoke/project.sge.ron`。
- 新增 `examples/projects/editor_smoke/scenes/main.scene.ron`。
- 新增 `examples/projects/editor_smoke/assets/asset_manifest.ron`。
- runtime 默认入口改为 project sample：scene path 为 `examples/projects/editor_smoke/scenes/main.scene.ron`，project root 为 `examples/projects/editor_smoke`。
- README 的 runtime/editor 示例命令使用 sample project 路径。
- runtime tests 不再把 `assets/examples/editor_smoke.scene.ron` 当默认真源。
- 若 `assets/examples/editor_smoke.scene.ron` 暂时保留，只作为 legacy fixture，并在 docs 中标注不是用户 project。
...
3. 实现 Repository Root Guard 和 absolute dialog path 到 project-relative path 的转换。
4. 实现 New/Open Project 文件创建、加载和 dirty switch pending action。
5. 对 Import/Save/Create/Open Scene 加 shared action gate。
6. 把 editor smoke 改为临时 project。
7. 增加或迁移一个 repo sample project 到 `examples/projects/`，并更新 runtime 默认入口。
