...
README、AGENTS、architecture overview和目标架构spec在M7 review通过后更新为M1–M7架构链路完成；延期项
包括但不限于audio/physics/network、archive/Pak/signing/installer、Play writeback、action remapping、
advanced gizmo handles/snapping/prefab、parallel ECS/RenderWorld/incremental Cook，完整清单与触发条件以目标架构规格为准，并保留
跨平台证据边界。
...
signing/installer、Play writeback、action remapping、advanced gizmo handles/snapping/prefab、parallel ECS/RenderWorld/incremental Cook，
