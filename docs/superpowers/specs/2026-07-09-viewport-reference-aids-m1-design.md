...
- 透视 editor camera 的 yaw/pitch、forward/right/up basis 同步切到 `Z-up`。
...
- 透视 editor camera 的默认 basis 为：`+X` forward、`+Y` right、`+Z` up。Yaw 绕 world `+Z`，pitch 绕 camera-local right。
- `W/A/S/D` 继续按 camera-local forward/right 移动，但这些向量必须从 `Z-up` basis 派生，不能继续使用旧的 `rotation * -Z` forward 或绕 `Y` yaw。
...
Imported OBJ 顶点在 M1 中按项目 world coordinates 解释，默认假定源文件已经是 `Z-up`。`asset::load_obj_mesh` 不做 `Y-up`/`Z-up` 自动转换；来自 Blender、Maya、3ds Max 等外部坐标系不同的 OBJ 如果方向不对，属于后续 asset import option，不在本 milestone 内解决。
...
orientation cube 点击行为必须同时定义 look direction 和 screen-up，避免同一方向出现多个合法 roll：
...
Derived screen-right must be deterministic from `look direction` and `screen up`; implementation should expose one helper that builds the orthographic `ViewportView` from this table instead of open-coding rotations per button.
...
| `editor::viewport` | editor-only `ViewCamera`/view preset、grid/axis overlay、orientation cube layout/hit test、orthographic view state；复用 render 暴露的 shared projection helper |
...
- Orientation cube 和 `Perspective` overlay 命中优先级高于 transform gizmo；transform gizmo 高于 scene mesh selection。点击 orientation overlay 不能触发 cube selection 或 clear selection。
...
Grid 和 axes 仍必须使用和 mesh draw-call 同一套 world-to-viewport projection。实施时从 `render` 提取或公开一个小的 shared projection helper，例如 `project_world_point(view, world_point) -> Option<[f32; 2]>` 或等价 value object。`editor::viewport` 只负责把 normalized viewport position 映射到 egui screen rect；不得复制一套与 `render::project_point` 不一致的投影公式。
...
- perspective navigation 使用 `Z-up` basis：yaw 绕 world `+Z`，pitch 绕 camera-local right，camera forward 默认指向 world `+X`。
- orthographic preset 生成 finite `ViewportView`。
- orthographic view 使用 `Projection::Orthographic`。
- orthographic preset 必须由 `look direction + screen up` 表生成，避免 roll 翻转。
...
- `editor::viewport` tests：
  ...
  - perspective camera forward/right/up basis 符合 `Z-up`，不再使用旧 `rotation * -Z` forward 或绕 `Y` yaw。
...
3. 明确 imported OBJ 边界：保持 loader 原样读顶点，文档和 tests 声明 source coordinates 按 `Z-up` project world 解释。
4. 删除或重建仓库内旧轴向 `.scene.ron` 示例和测试输入。
5. 扩展 editor-only view state，支持 perspective 和六个 orthographic preset；preset helper 使用固定 look direction + screen-up 表。
6. 增加 orientation cube layout、paint 和 hit test，返回 `SetViewPreset` action；点击优先于 gizmo 和 scene selection。
7. 增加 fixed `X-Y` grid、major axes 和 `Z` axis marker overlay，并通过 shared projection helper 绘制。
8. 接入 `EditorApp` action handling，确保 preset click 不置 dirty，Pilot Camera active 时不改 selected camera，New/Open/reopen reset editor-only view state。
9. 更新 README 和 architecture overview 中的当前实现、smoke 边界和 `Z-up` 约定。
10. 运行最小自动验证和一次人工 viewport smoke。
