...
- Editor/Player 的 WGPU/CPU 后端共用 owned `RenderSnapshot`、`RenderView`、`RuntimeAssetStore`、投影和`RenderSettings`合同；两种后端均支持Lit、Unlit、隐藏线Wireframe与Lit+Wireframe。Wireframe执行相同几何depth预通过后仅叠加通过`LessEqual`的原始三角边，Lit+Wireframe复用Lit depth，遮挡边不透出。
