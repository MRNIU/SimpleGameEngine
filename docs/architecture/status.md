...
- canonical OBJ/PNG import、rebuildable cache、dependency closure、full Cook 和 source-free mesh/texture runtime products。
- PNG 以独立稳定 `AssetId`/`sge.texture` 进入Editor import、Reflect Material引用、Cook与Stage；损坏PNG、缺失/错误类型引用和缺失UV均fail closed。颜色纹理按sRGB解码，Lit/Unlit在CPU/WGPU中以repeat+bilinear采样并在线性空间乘颜色factor，WGPU纹理资源按`AssetId` retained。
...
- `EditSession`、通用 Inspector/history、entity workflow、文件工作流和 isolated Play；Transform rotation 在 Inspector 中以 X（Roll）、Y（Pitch）、Z（Yaw）欧拉角度数编辑，底层四元数合同不变；dirty close的Save/Discard/Cancel及Save失败恢复边界已有状态回归。
...
- 通用MTL/glTF importer、normal/roughness/metallic等PBR纹理、Phong/PBR、多光源、阴影和高级渲染尚未形成产品纵切；当前只支持显式AssetId引用的颜色PNG。
...
- Linux/Xvfb 下 Editor authoring、Play input、WGPU callback、WGPU/CPU与四种render mode实时切换、project/manifest/scene逐字节不变断言、简体中文CJK字体加载与1280×720界面readback，以及两种 Player backend 的 surface present/readback；Kenney Factory Kit 3.0纹理主体的双后端语义readback证明两个既有UV位置产生不同的非factor颜色且主体完整留在frame内。render crate另有CPU/WGPU纹理sRGB/repeat/bilinear/factor容差测试、四模式readback、Wireframe远端/背向边、Lit Wireframe隐藏边与Retina线宽合同测试。性能采样另有确定性rolling-window/percentile/跳帧语义测试和真实Player双帧报告证据。
- Editor PNG import、Reflect设置、save/reopen、Play、full Cook、真实 Cargo Player build、copied source-free Stage、删除source project和staged Player present；PNG修改使generation失效，未变输入保持确定输出。
