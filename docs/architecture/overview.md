...
| `sge-asset`（`crates/sge-asset/`） | 正式 UUID `AssetId`、typed `AssetRef<T>`、canonical `MeshAsset`、strict runtime catalog domain 和只读 `AssetLookup`；尚无 source import、Cook、runtime content store 或 GPU handle |
