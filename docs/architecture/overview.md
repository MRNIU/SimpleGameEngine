...
| `sge-asset`（`crates/sge-asset/`） | 正式 UUID `AssetId`、typed `AssetRef<T>`、canonical `MeshAsset` runtime product 和只读 `AssetLookup`；尚无 source import、Cook、runtime catalog/store 或 GPU handle |
