...
| `render` | ECS render extraction、`wgpu 29` viewport pipeline、primitive cube 和 imported mesh draw call、mesh span metadata |
| `window` | winit window config |
| `input` | keyboard/mouse state |
| `editor` | egui menu/toolbar/status/panels、Unreal-like 三栏布局、固定快捷键、Inspector scene content 编辑、系统文件对话框 `.scene.ron` Open/Save As 和 OBJ import、Assets 区、session imported mesh cache、Undo/Redo、egui-wgpu callback viewport、editor-only viewport camera、Pilot Camera、viewport selection、Move/Scale transform gizmo |
| `runtime` | scene load、显式 project-root manifest/OBJ 解析和 viewport draw smoke |
...
当前 editor smoke 通过退出码和 `editor smoke ok: ... transform_undo_redo=..., content_reopen=..., history_cleared=..., gizmo_drag_cleared=..., pilot_camera_cleared=..., assets=..., imported_meshes=..., imported_asset_reopened=..., imported_viewport_span=..., viewport_prepare=..., viewport_paint=...` summary log 确认文件工作流 save/open 闭环、内部 OBJ import、manifest/cache、`asset:<uuid>` reopen、imported mesh viewport span、gizmo semantic preview/commit/Undo/Redo、material/light/camera 参数 smoke、editor-only state 清理和真实 `ViewportRenderer` path 触达；它不做 OS 级鼠标键盘自动点击、截图、像素检查、真实系统文件对话框或真实 GPU 兼容性证明。人工 host-native GUI smoke 已作为手动证据层通过，但仍不等于跨平台 GPU 兼容性证明，也不进入默认 CI gate。
