...
| `editor` | egui toolbar/panels、inspector、hierarchy、`.scene.ron` 文件工作流、Undo/Redo、egui-wgpu callback viewport、editor-only viewport camera、viewport selection、Move/Scale transform gizmo |
...
当前 editor smoke 通过退出码和 `editor smoke ok: ... viewport_prepare=..., viewport_paint=...` summary log 确认文件工作流 save/open 闭环和真实 `ViewportRenderer` path 触达；它不做截图、像素检查或真实 GPU 兼容性证明。人工 host-native GUI smoke 已作为手动证据层通过，但仍不等于跨平台 GPU 兼容性证明，也不进入默认 CI gate。
