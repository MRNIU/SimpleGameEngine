...
`editor` 拥有 editor-only `Z-up` viewport input/camera、自适应 `XY` / `XZ` / `YZ` world grid、XYZ axis、orientation cube、camera hint 和 transform gizmo state。`render` 接收 `ViewportView` 和 world-space draw-call vertices，统一生成 view-projection matrix、homogeneous clipping、world ray，并通过 offscreen color + `Depth32Float` depth pass 渲染后 composite 到 egui；selection、grid、gizmo 和 fallback 共用同一个 projection 合同。
