...
| `sge-asset`（`crates/sge-asset/`） | 正式 UUID `AssetId`、typed `AssetRef<T>`、canonical `MeshAsset`、strict runtime catalog、verified content root 和 `RuntimeAssetStore`；尚无 source import、Cook 或 GPU handle |
...
当前正在实现 **Asset Pipeline And Runtime Products (M3)**。canonical MeshAsset、strict runtime catalog/content/store 已实现；OBJ importer 迁移、import cache、Cook、RuntimeScene，以及后续 `RenderSnapshot`、game-specific Editor/Player、`PlaySession`、Build/Stage 和最终 integration demo 尚未实现。
