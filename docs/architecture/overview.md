...
目标架构见 `docs/superpowers/specs/2026-07-11-rust-engine-target-architecture-design.md`。Core Kernel M1、Project And Data M2 与 Asset Pipeline And Runtime Products M3 已完成 headless 实现与验证；当前 Editor/Scene/Render/runtime 产品路径仍使用 bare prototype 数据流，尚未迁移到 target path。
...
| `sge-asset`（`crates/sge-asset/`） | 正式 UUID `AssetId`、typed `AssetRef<T>`、canonical `MeshAsset`、strict runtime catalog、verified content root 和 `RuntimeAssetStore`；不拥有 source import、Cook orchestration 或 GPU handle |
| `sge-project`（`crates/project/`） | strict `ProjectDescriptor`、portable `ProjectPath` / `ProjectRoot`、manifest v2 / OBJ import settings、source record、directory containment 与单文件 atomic replace；不拥有 Editor session 或 importer |
| `sge-scene`（`crates/sge-scene/`） | strict authoring/runtime DTO、`SceneEntityId` / `Parent`、共享 validation/prepare、runtime build、`instantiate` / `SceneInstance` 和 `snapshot`；不拥有 project/Cook I/O、Editor session 或 GPU |
| `sge-asset-pipeline`（`crates/sge-asset-pipeline/`） | canonical OBJ importer、可重建 import cache、dependency closure、deterministic full Cook、immutable generation 与 atomic catalog publication；不拥有 App/Editor/Player host、render/UI/GPU 或 Cargo build |
...
M3 headless products 已闭合。下一里程碑是 **Render And Hosts (M4)**：`RenderSnapshot`、target WGPU render path、game-specific Editor/Player host 与 `PlaySession` 尚未实现；Build/Stage 和最终 integration demo 也仍属于后续里程碑。M3 的 `RuntimeContentRoot` 不是 Player，Cook output 也不是 Stage。
...
Linux CI 还调用 `scripts/audit-boundaries.sh` 执行 fail-closed target dependency、production source 与 retained bare OBJ caller audit；本地使用同一入口。
...
M3 focused/headless 证据：

- `sge-asset-pipeline` tests 覆盖 canonical OBJ import、cache hit/rebuild、full Cook closure、precommit consumer barrier、atomic publication、相同输入和 cache 重建后的 deterministic tree。
...
上述既有 smoke 不代表 Editor/Player 已接入 `EngineApp` 或 target path，也不证明 Player、Stage、最终 demo 或新增平台 GUI 行为。M3 不迁移窗口、Editor 或 renderer，因此没有新增 GUI/Xvfb/WGPU smoke。
