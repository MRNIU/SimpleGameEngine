...
- M1–M7 已完成：typed ECS / Reflect / EngineApp、strict project/authoring data、canonical OBJ/PNG import/full Cook/runtime products、owned `RenderSnapshot`、WGPU/CPU 双渲染后端、Edit/Play target Editor 架构路径、game-specific Build/self-contained Stage 和最终 integration demo 已形成一条产品路径。
...
- `examples/demo_game/` 默认使用 [Kenney Factory Kit 3.0](https://kenney.nl/assets/factory-kit) 的 CC0 `conveyor-bars-stripe.obj` 与 `Textures/colormap.png`，通过两个固定 `AssetId` 显式组成带纹理主体；无纹理颜色factor方块由正式 Basic Shapes 路径生成。内置 source、版本、hash 与许可记录在 `Content/ThirdParty/KenneyFactoryKit/`，New Project、Editor、Play 和 Build 全程离线复制或读取仓库内容。同一 game plugin 在 headless、Editor Play、Player 与 Cook validation 运行。
- PNG 以 `sge.texture` 进入 manifest；Editor 的 Import PNG 采用 create-only source、candidate validation 与 manifest atomic save，失败不提交 source、manifest、live session 或 scene history。`Material` 只保存可选强类型 `AssetRef<TextureAsset>` 与线性 base-color factor，不保存 PNG 字节、路径或 GPU handle。
- Cook 将被 scene 引用的 mesh/texture dependency closure 发布为 source-free runtime products。Lit/Unlit 使用 OBJ 既有 `vt`，按 repeat + bilinear 采样 sRGB texture，在线性空间乘 factor；Wireframe 继续遵守既有无 fill 合同。缺失/错误类型 texture、损坏 PNG 与纹理 mesh 缺失 UV 均 fail closed。
...
Editor打开project时会生成ignored import cache；Build输出位于ignored `build/`。Player从Stage同级runtime自定位，不需要source project、OBJ/PNG importer或source `Content`。
...
| `crates/sge-asset/` | AssetId、MeshAsset、TextureAsset、runtime catalog/content/store |
...
| `crates/sge-asset-pipeline/` | OBJ/PNG import、cache、full Cook |
