...
- `editor` viewport 使用 `Z-up` editor camera、自适应十进制 world grid、XYZ axis、orientation cube、camera speed/FOV/distance hint 和 Move/Rotate/Scale gizmo。Perspective 默认水平 FOV 为 `90°`，按实际 viewport aspect 换算垂直 FOV，并使用随相机移动/高度扩展的 world-plane material grid；Orthographic preset 使用对应的 `XY` / `XZ` / `YZ` 自适应 line grid。两条 grid 路径与 scene mesh 在同一个 WGPU depth pass 中渲染，不作为覆盖物遮挡 mesh。
