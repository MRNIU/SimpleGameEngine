...
fn plugin_registers_exact_scene_saveable_schema() -> Result<(), Box<dyn std::error::Error>> {
    let app = ready_app()?;
    let expected = [
        ("sge.transform", vec!["rotation", "scale", "translation"]),
        (
            "sge.camera",
            vec![
                "active",
        ...
        ("sge.mesh_renderer", vec!["mesh"]),
        ("sge.material", vec!["base_color"]),
        ("sge.light", vec!["color", "intensity"]),
    ];

    for (type_key, fields) in expected {
        let descriptor = app
            .type_registry()
            .descriptor(type_key)
...
fn component_codecs_roundtrip_private_values() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let material = Material::new([0.2, 0.4, 0.6, 1.0]);
